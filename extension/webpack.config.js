		...
		optimization: {
			...
			minimizer: [
				new TerserPlugin({
					...
					extractComments: {
						condition: /^\**!|@preserve|@license|@copyright/i,
						filename: 'THIRD_PARTY_NOTICES.txt',
						banner: 'Third-party license notices are in THIRD_PARTY_NOTICES.txt'
					}
				})
			],
			moduleIds: 'named',
		...
		plugins: [
			new CopyPlugin({
				patterns: [
					...
					// providers embutidos: a lista local e a base, o fetch remoto so complementa
			...
			...(isProduction ? [
				new ZipPlugin({
					...
					filename: `corebrain-clipper-${package.version}-${browserName}.zip`,
				})
			] : [])
		]
	};
