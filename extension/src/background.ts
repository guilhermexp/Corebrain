...
import { estaLogadoNoX, buscarBookmarks, observarQueryId, salvarComoNotas, pluginDisponivel, enviarPelaPonte, DEFAULT_X_BOOKMARK_FOLDER, podeAvancarMarcador } from './utils/x-bookmarks';
import { incrementStat, loadSettings } from './utils/storage-utils';
import { precisaFallbackWebRequest } from './utils/webrequest-guard';
...
if (typeof browser !== 'undefined' && precisaFallbackWebRequest({
	webRequest: browser.webRequest?.onBeforeSendHeaders,
	declarativeNetRequest: (globalThis as any).chrome?.declarativeNetRequest,
})) {
	try {
...
let xSyncCancelado = false;
// Aba dedicada do sync em lote. Sem isso, cada nota navegaria a aba ATIVA do
// usuario para obsidian:// — o que sequestra a pagina que ele esta vendo e
// dispara o CSP do site (foi o erro visto no x.com).
let xSyncTabId: number | undefined;

async function abrirUrlNaAbaDoSync(url: string): Promise<void> {
	try {
		if (xSyncTabId !== undefined) {
			await browser.tabs.get(xSyncTabId); // confirma que ainda existe
		}
	} catch {
		xSyncTabId = undefined;
	}
	if (xSyncTabId === undefined) {
		const aba = await browser.tabs.create({ url: 'about:blank', active: false });
		xSyncTabId = aba.id;
	}
	if (xSyncTabId !== undefined) await browser.tabs.update(xSyncTabId, { url });
}

async function fecharAbaDoSync(): Promise<void> {
	if (xSyncTabId === undefined) return;
	try { await browser.tabs.remove(xSyncTabId); } catch { /* ja fechada */ }
	xSyncTabId = undefined;
}
...
browser.runtime.onMessage.addListener((request: unknown, sender: browser.Runtime.MessageSender, sendResponse: (response?: any) => void): true | undefined => {
	if (typeof request === 'object' && request !== null) {
		...
		// --- sync dos bookmarks do X ---
		// Roda aqui, no service worker, e nao no popup: o popup fecha assim que
		// perde o foco e mataria a paginacao no meio.
		if (typedRequest.action === 'x-bookmarks-status') {
			estaLogadoNoX()
				.then((logado) => sendResponse({ logado }))
				.catch((e) => sendResponse({ logado: false, error: (e as Error).message }));
			return true;
		}

		if (typedRequest.action === 'x-bookmarks-cancelar') {
			xSyncCancelado = true;
			sendResponse({ ok: true });
			return true;
		}

		// busca + grava, reportando as duas fases pro popup
		if (typedRequest.action === 'x-bookmarks-sync-e-salvar') {
			(async () => {
				try {
					xSyncCancelado = false;
					const st = await browser.storage.local.get(['x_bookmarks_last_id', 'x_bookmarks_pasta']);
					const ultimo = st.x_bookmarks_last_id as string | undefined;
					const pasta = (st.x_bookmarks_pasta as string) || DEFAULT_X_BOOKMARK_FOLDER;
					// An invalid token is an actionable pairing error, not an unavailable
					// plugin. Check before contacting X so we never do paid/network work
					// only to discard the batch after an auth failure.
					const ponteDisponivel = await pluginDisponivel();

					const r = await buscarBookmarks(ultimo, (qtd, pagina) => {
						browser.runtime.sendMessage({ action: 'x-bookmarks-progress', qtd, pagina }).catch(() => {});
					}, Infinity, () => xSyncCancelado);

					if (xSyncCancelado) return sendResponse({ success: true, salvos: 0, erros: 0, cancelado: true });

					const settings = await loadSettings();
					const vault = settings.vaults?.[0] || '';
					// Preferimos a ponte com o plugin: sem ela, cada nota ativaria o
					// Obsidian e o usuario nao consegue nem trocar de janela.
					let res: { salvos: number; pulados: number; erros: number };
					if (ponteDisponivel) {
						browser.runtime.sendMessage({
							action: 'x-bookmarks-saving', feitos: 0, total: r.novos.length, viaPlugin: true,
						}).catch(() => {});
						const p = await enviarPelaPonte(r.novos, pasta);
						res = { salvos: p.criados, pulados: p.pulados, erros: p.erros };
					} else {
						const fallback = await salvarComoNotas(r.novos, pasta, vault, abrirUrlNaAbaDoSync, (feitos, total) => {
							browser.runtime.sendMessage({ action: 'x-bookmarks-saving', feitos, total }).catch(() => {});
						});
						res = { ...fallback, pulados: 0 };
					}

					// Advance only when every fetched bookmark was either created or
					// already present. Partial failures must remain pending next run.
					if (podeAvancarMarcador(r.novos.length, res.salvos, res.pulados, res.erros, r.alcancouFim)) {
						await browser.storage.local.set({ x_bookmarks_last_id: r.novos[0].id });
					}
					await fecharAbaDoSync();
					sendResponse({
						success: true,
						...res,
						encontrados: r.novos.length,
						incompleto: !r.alcancouFim,
						paginas: r.paginas,
					});
				} catch (e) {
					await fecharAbaDoSync();
		...
		if (typedRequest.action === 'x-bookmarks-sync') {
			(async () => {
				try {
					const st = await browser.storage.local.get('x_bookmarks_last_id');
					const ultimo = st.x_bookmarks_last_id as string | undefined;
					...
					// so avanca o marcador se veio algo, senao um sync vazio perderia o ponto
					if (r.novos.length) {
						await browser.storage.local.set({ x_bookmarks_last_id: r.novos[0].id });
					}
		...
		// lote: nunca toca na aba do usuario
		if (typedRequest.action === "openObsidianUrlEmLote") {
			abrirUrlNaAbaDoSync((typedRequest as any).url)
				.then(() => sendResponse({ success: true }))
		...
		if (typedRequest.action === "extractContent" ||
			...
			typedRequest.action === "openObsidianUrlEmLote") {
			return true;
		}
	}
	return undefined;
...
// captura o queryId de Bookmarks toda vez que o x.com chama a propria API
observarQueryId();
