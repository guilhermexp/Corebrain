...
/**
 * providers.json empacotado na extensao. E a base da lista: os providers daqui
 * sempre existem e vencem o remoto, entao os nossos (Kimi/GLM coding plan) nao
 * somem quando o upstream publica a lista dele.
 */
async function getBundledProviders(): Promise<ProviderPresets | null> {
	try {
		const response = await fetch(browser.runtime.getURL('providers.json'));
		if (!response.ok) return null;
		return await response.json() as ProviderPresets;
	} catch (error) {
		console.error('Failed to read bundled providers:', error);
		return null;
	}
}

function toProviderMap(data: ProviderPresets): Record<string, PresetProvider> {
...
async function fetchPresetProviders(): Promise<Record<string, PresetProvider>> {
	...
	const bundled = await getBundledProviders();
	try {
		const response = await fetch(PROVIDERS_URL);
		if (!response.ok) {
			throw new Error(`HTTP error! status: ${response.status}`);
		}
		const remote = await response.json() as ProviderPresets;
		const data = { ...remote, ...(bundled || {}) } as ProviderPresets;

		await setLocalStorage(LOCAL_STORAGE_KEY, data);
		debugLog('Providers', 'Stored providers in local storage:', data);

		const providers = toProviderMap(data);
		debugLog('Providers', 'Successfully fetched presets:', providers);
		return providers;
	} catch (error) {
		console.error('Failed to fetch preset providers:', error);
		// offline ou upstream fora do ar: a lista embutida ainda serve
		if (bundled) {
			debugLog('Providers', 'Falling back to bundled providers');
			await setLocalStorage(LOCAL_STORAGE_KEY, bundled);
			return toProviderMap(bundled);
		}
		throw error;
	}
}
