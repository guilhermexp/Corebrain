...
import { initXSync } from '../utils/x-sync-ui';
import { jaNaGaleria, clipPelaPonte, BridgeAuthenticationError } from '../utils/x-bookmarks';
...
export function getPropertiesFromDOM(): Property[] {
	// inclui textarea: a description e colapsavel e usa textarea em vez de input
...
let duplicataConfirmada = false;

function mostrarAvisoDuplicata(caminho: string | null): void {
	const el = document.getElementById('aviso-duplicata');
	if (!el) return;
	const txt = el.querySelector('.aviso-duplicata-texto') as HTMLElement | null;
	if (txt) txt.textContent = caminho ? `${getMessage('jaNaGaleria')} (${caminho})` : getMessage('jaNaGaleria');
	el.style.display = 'flex';
}

function esconderAvisoDuplicata(): void {
	const el = document.getElementById('aviso-duplicata');
	if (el) el.style.display = 'none';
}

/**
 * Avisa uma vez que a URL ja esta na galeria; no segundo clique deixa salvar.
 *
 * Se o plugin nao responde (Obsidian fechado), jaNaGaleria devolve null e a
 * gente SEGUE: travar o clip porque a checagem falhou seria pior que a
 * duplicata que ela evita.
 */
async function passouNaChecagemDeDuplicata(): Promise<boolean> {
	if (duplicataConfirmada) return true;
	const url = currentTabId ? (await getTabInfo(currentTabId)).url || '' : '';
	const dup = await jaNaGaleria(url);
	if (!dup?.existe) {
		esconderAvisoDuplicata();
		return true;
	}
	mostrarAvisoDuplicata(dup.caminho);
	duplicataConfirmada = true;
	return false;
}
...
function buildTemplateFieldsSkeleton(template: Template | null) {
	...
	if (Array.isArray(template.properties)) {
		for (const property of template.properties) {
			...
			// A description costuma ser o campo mais longo do template. Vira textarea
			// colapsada: continua visivel e editavel, mas nao come o popup inteiro.
			const ehLonga = property.name === 'description' && propertyType === 'text';
			const inputElement = document.createElement(ehLonga ? 'textarea' : 'input') as HTMLInputElement;
			...
			if (ehLonga) {
				propertyDiv.classList.add('is-colapsavel', 'is-colapsada');
				(inputElement as unknown as HTMLTextAreaElement).rows = 1;
				metadataPropertyKey.addEventListener('click', () => {
					propertyDiv.classList.toggle('is-colapsada');
				});
			} else {
...
function setupMetadataToggle() {
	...
	if (metadataHeader && metadataProperties) {
		...
		getLocalStorage('propertiesCollapsed').then((isCollapsed) => {
			updateMetadataToggleState(estadoInicialColapsado(isCollapsed));
		});
	}
}

/**
 * Padrao colapsado: o bloco de propriedades ocupa quase todo o popup e na maioria
 * dos clips nao precisa ser editado. Assim que o usuario abre uma vez, a escolha
 * dele passa a valer (fica salva em propertiesCollapsed).
 */
export function estadoInicialColapsado(valorSalvo: unknown): boolean {
	return typeof valorSalvo === 'boolean' ? valorSalvo : true;
}
...
function determineMainAction() {
	...
	// Sync dos bookmarks do X: entra como item COM TEXTO no menu. Como icone solto
	// no header ele ficava indistinguivel dos outros cinco.
	const addSyncX = () => addSecondaryAction(secondaryActions, 'syncXBookmarks', () => {
		(document.getElementById('x-sync') as HTMLElement | null)?.click();
	});
...
async function handleClipObsidian(): Promise<void> {
	...
	// antes de qualquer trabalho: essa URL ja esta na galeria? Auth failure is
	// actionable pairing feedback, not permission to fall back to obsidian://.
	try {
		if (!(await passouNaChecagemDeDuplicata())) return;
	} catch (error) {
		showError(error instanceof BridgeAuthenticationError ? error.message : 'failedToSaveFile');
		return;
	}
	...
	try {
		...
		// Primeiro a ponte: escreve a nota sem ativar o Obsidian nem abrir a nota,
		// entao voce continua na pagina que estava lendo. Se ela nao responder
		// (Obsidian fechado, plugin desligado), cai no `obsidian://` de sempre.
		const botao = document.getElementById('clip-btn');
		botao?.classList.add('clipping');
		const pelaPonte = isDailyNote ? null : await clipPelaPonte(fileContent, noteName, path);
		botao?.classList.remove('clipping');
		if (!pelaPonte) {
...
function getActionIcon(actionType: string): string {
	switch (actionType) {
		...
		case 'syncXBookmarks': return 'bookmark';
		default: return 'plus';
	}
}
