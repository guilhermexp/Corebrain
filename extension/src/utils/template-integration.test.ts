...
// The fixture intentionally exercises {{date}}, whose offset follows the
// installer's local timezone. Keep the semantic instant stable without making
// contributors or CI set TZ before running the suite.
function expectedForEnvironment(name: string, expected: string): string {
	if (name !== 'youtube') return expected;
	const localCreated = dayjs(FROZEN_DATE).format('YYYY-MM-DDTHH:mm:ssZ');
	return expected.replace(/created: "[^"]+"/, `created: "${localCreated}"`);
}
...
describe('Template fixtures', () => {
	...
	test.each(fixtures)('$name', async ({ name, jsonPath, htmlPath }) => {
		...
		expect(result.trim()).toEqual(expectedForEnvironment(name, expected).trim());
	});
});
