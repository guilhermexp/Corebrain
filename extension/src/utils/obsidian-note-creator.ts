...
function openObsidianUrl(url: string, emLote = false): void {
	browser.runtime.sendMessage({
		// em lote vai pra uma aba dedicada; navegar a aba ativa sequestraria a
		// pagina do usuario e esbarra no CSP do site (visto no x.com)
		action: emLote ? "openObsidianUrlEmLote" : "openObsidianUrl",
		url: url
	}).catch((error) => {
...
export async function saveToObsidian(
	...
	// Importacao em lote (ex.: bookmarks do X) precisa gravar sem abrir a nota:
	// sem isso, N notas viram N janelas abertas de uma vez.
	forcarSilencioso = false,
): Promise<void> {
	...
	if (generalSettings.silentOpen || forcarSilencioso) {
		obsidianUrl += '&silent=true';
	}

	// Em lote nao da pra usar a area de transferencia: ela e um recurso unico e
	// as notas se atropelariam. Manda o conteudo na propria URI.
	if (forcarSilencioso) {
		obsidianUrl += `&content=${encodeURIComponent(fileContent)}`;
		openObsidianUrl(obsidianUrl, true);
		return;
	}
